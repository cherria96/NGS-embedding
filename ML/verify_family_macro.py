...
from family_macro_models import TARGETS

def main():
    parser=argparse.ArgumentParser();parser.add_argument('--output-dir',type=Path,default=Path(__file__).resolve().parent/'results_family_macro');args=parser.parse_args();out=args.output_dir
    ...
    sources=[]
    for d in ['BAC','ARC']:
        f=pd.read_excel(Path(__file__).resolve().parents[1]/f'data/final/Dat_{d}.xlsx',sheet_name='F(%)').set_index('Family').T
        f.index=['-'.join(c.split('-')[:2]) for c in f.index];f.columns=[d+'_F::'+c for c in f.columns];sources.append(f)
