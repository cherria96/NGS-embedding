...
BLOCKS = {'BAC_P': 'Phylum', 'BAC_O': 'Order', 'ARC_O': 'Order', 'ARC_G': 'Genus'}
...
class SampleAbundanceTaxa(BaseEstimator, TransformerMixin):
    """Strict per-sample >0.1 percent mask; fit union on training rows only."""
    ...
    def transform(self, X):
        ...
        for block, columns in self.selected_.items():
            parts.append(X[columns].where(self.mask(X, block)[columns], 0))
