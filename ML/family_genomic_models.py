...
CATS = []
NUMS = []
BLOCKS = {'BAC_F': 'Family', 'ARC_F': 'Family'}
...
class SampleAbundanceTaxa(BaseEstimator, TransformerMixin):
    """Family >1 percent in at least 5 percent of training samples; fourth root."""
    def mask(self, X, block):
        ...
        return pd.DataFrame(values > 1.0, index=X.index, columns=columns)

    def fit(self, X, y=None):
        if any(not c.startswith(('BAC_F::', 'ARC_F::')) for c in X.columns):
            raise ValueError('Genomic-only input must contain Family features exclusively')
        self.selected_, self.counts_ = {}, {}
        for block in BLOCKS:
            counts = self.mask(X, block).sum()
            self.selected_[block] = counts[counts / len(X) >= .05].index.tolist()
            self.counts_[block] = counts
        self.columns_ = CATS + NUMS + sum(self.selected_.values(), [])
        return self

    def transform(self, X):
        parts = [X[CATS + NUMS].copy()]
        for block, columns in self.selected_.items():
            parts.append(X[columns].where(self.mask(X, block)[columns], 0).pow(.25))
