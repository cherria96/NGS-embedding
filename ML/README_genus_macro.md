# Genus microbiome-only prediction with five-label macro metrics

Run code: [train_genus_macro.py](train_genus_macro.py). Results: [report.md](results_genus_macro/report.md).
...
/home/best/anaconda3/envs/tsf-ad/bin/python ML/train_genus_macro.py --seed 42 --n-iter 8

OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 MPLCONFIGDIR=/tmp/ngs-mpl \
/home/best/anaconda3/envs/tsf-ad/bin/python -m unittest discover -s ML -p test_genus_macro.py

/home/best/anaconda3/envs/tsf-ad/bin/python ML/verify_genus_macro.py
```
...
Genus integration is implemented in `genus_macro_data.py`; taxon-independent splitting and search utilities are reused from `train_family_point1.py`;
no prior model scores or selected taxa are read. G(%) sheets supply original Genus
percentages; taxa are checked against OTUs Genus annotations. Workbook IDs are
...
All supplied Genus annotation bins, including unidentified/off-domain annotations,
...
- `selected_taxa.csv`, `selected_Bacteria_genera.csv`, `selected_Archaea_genera.csv`,
  `final_feature_list.txt`, `taxa_audit.csv`, `cv_feature_columns.csv`: full feature
  lists, domains, Genus level, per-taxon counts, and training/fold inclusion.
- `sample_audit.csv`, `sample_id_mapping.csv`, `sample_id_corrections.csv`,
  `file_structure.csv`, `genus_source_counts.csv`, `samples_per_site.csv`,
...
- `*.joblib`, `*_preprocessing.joblib`: trained models and preprocessing. Load with
  ML on PYTHONPATH and supply raw Genus percentages, not standardized values.
