...
from genus_macro_models import TARGETS

def main():
    parser=argparse.ArgumentParser();parser.add_argument('--output-dir',type=Path,default=Path(__file__).resolve().parent/'results_genus_macro');args=parser.parse_args();out=args.output_dir
    ...
    sources=[]
    for d in ['BAC','ARC']:
        f=pd.read_excel(Path(__file__).resolve().parents[1]/f'data/final/Dat_{d}.xlsx',sheet_name='G(%)').set_index('Genus').T
        f.index=['-'.join(c.split('-')[:2]) for c in f.index];f.columns=[d+'_G::'+c for c in f.columns];sources.append(f)
    ...
    assert set(taxa.taxonomic_level)=={'Genus'}
