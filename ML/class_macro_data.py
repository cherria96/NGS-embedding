"""Audited Class workbook integration; metadata is never a predictor."""
import numpy as np
import pandas as pd
from class_macro_models import TARGETS, CATS, NUMS, numeric
from class_macro_utils import label_report
...
def inspect(data_dir, out, group_map):
    ...
    for domain in ['BAC', 'ARC']:
        ...
        frame = pd.read_excel(book, sheet_name='C(%)').set_index('Class')
        if frame.index.isna().any() or frame.index.duplicated().any():
            raise ValueError('Missing/duplicate Class names')
        if set(frame.index) != set(otu.Class.fillna('unidentified')):
            raise ValueError('Class sheet and OTU Class annotation disagree')
        frame = frame.apply(pd.to_numeric, errors='raise')
        if not np.isfinite(frame.values).all() or (frame.values < 0).any():
            raise ValueError('Missing or invalid Class percentage: cannot assume absence')
        if not np.allclose(frame.sum(axis=0), 100, atol=.1):
            raise ValueError('Class percentages do not sum to approximately 100')
        ...
        for full in original:
            ...
            mappings.append({'domain': domain, 'rank': 'Class', 'workbook_sample': full,
        ...
        frame.columns = ids
        frame = frame.T
        class_source.append({'domain': domain, 'original_OTUs': len(otu),
                              'unique_Classes': len(frame.columns)})
        frame.columns = [f'{domain}_C::{taxon}' for taxon in frame.columns]
        sheets[f'{domain}_C'] = frame
        book.close()
    pd.DataFrame(class_source).to_csv(out/'class_source_counts.csv', index=False)
