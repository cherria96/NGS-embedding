...
class SampleAbundanceTaxa(BaseEstimator, TransformerMixin):
    """Training-only union of sample-wise strict >0.1 percent Class abundances."""
    ...
    def fit(self, X, y=None):
        if any(not c.startswith(('BAC_C::', 'ARC_C::')) for c in X.columns):
            raise ValueError('Microbiome-only input must contain Class features exclusively')
