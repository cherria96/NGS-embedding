...
def main():
    ...
    sources=[]
    for d in ['BAC','ARC']:
        f=pd.read_excel(Path(__file__).resolve().parents[1]/f'data/final/Dat_{d}.xlsx',sheet_name='P(%)').set_index('Phylum').T
        f.index=['-'.join(c.split('-')[:2]) for c in f.index];f.columns=[d+'_P::'+c for c in f.columns];sources.append(f)
    ...
    assert set(taxa.taxonomic_level)=={'Phylum'}
