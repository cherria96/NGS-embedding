...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--output-dir',type=Path,default=Path(__file__).resolve().parent/'results_order_macro');args=parser.parse_args();out=args.output_dir
    ...
    sources=[]
    for d in ['BAC','ARC']:
        f=pd.read_excel(Path(__file__).resolve().parents[1]/f'data/final/Dat_{d}.xlsx',sheet_name='O(%)').set_index('Order').T
        f.index=['-'.join(c.split('-')[:2]) for c in f.index];f.columns=[d+'_O::'+c for c in f.columns];sources.append(f)
    ...
    assert set(taxa.taxonomic_level)=={'Order'}
