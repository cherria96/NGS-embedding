#!/usr/bin/env python3
"""Site-grouped joint warning comparison; see README_family_point1.md."""
...
from family_point1_models import TARGETS, CATS, NUMS, BLOCKS, SampleAbundanceTaxa, WarningModel, numeric
...
def family_summary(out, taxa, n_all, n_train):
    lines = [f'Union of families exceeding 0.1% in at least one of {n_all} matched samples. Values <=0.1% are zero; other values remain original percentages. No renormalization, root transform, or prevalence filter.']
    for block, frame in taxa.groupby('block'):
        keep = frame[frame.included_in_final_matrix]
        keep.to_csv(out/f"retained_{block.split('_')[0]}_families.csv", index=False)
        lines += [f'\n## {block}: {len(keep)} features', '| Family | Samples >0.1% |', '|---|---:|']
        lines += [f"| {r.taxon} | {r.samples_above_0_1_percent} |" for r in keep.itertuples()]
    lines += [f'\nTotal microbiome features: {int(taxa.included_in_final_matrix.sum())}.']
...
def inspect(data_dir, out, group_map):
    ...
    for domain in ['BAC', 'ARC']:
        ...
        frame = pd.read_excel(book, sheet_name='F(%)').set_index('Family')
        if frame.index.isna().any() or frame.index.duplicated().any():
            raise ValueError('Missing/duplicate Family names')
        if set(frame.index) != set(otu.Family.fillna('unidentified')):
            raise ValueError('Family sheet and OTU Family annotation disagree')
        frame = frame.apply(pd.to_numeric, errors='raise')
        if not np.isfinite(frame.values).all() or (frame.values < 0).any():
            raise ValueError('Missing or invalid Family percentage: cannot assume absence')
        if not np.allclose(frame.sum(axis=0), 100, atol=.1):
            raise ValueError('Family percentages do not sum to approximately 100')
...
def main():
    parser = argparse.ArgumentParser(description='Joint five-label analysis with strict sample-wise Family >0.1% filtering')
    parser.add_argument('--data-dir', type=Path, default=ROOT/'data/final')
    parser.add_argument('--output-dir', type=Path, default=ROOT/'ML/results_family_point1')
    ...
    # Predeclared union across matched samples, expressly permitted by the protocol.
    # This is the only all-sample feature-space operation; no outcome enters it.
    source_X = X.copy()
    ...
    # Audit fixed all-sample union and training counts separately.
    rows = []
    for block in BLOCKS:
        counts = selector.mask(source_X, block).sum()
        for c in counts.index:
            rows.append({'block': block, 'taxon': c.split('::', 1)[1], 'feature': c,
                         'samples_above_0_1_percent': int(counts[c]),
                         'training_samples_above_0_1_percent': int((Xtr[c] > .1).sum()) if c in Xtr else 0,
                         'all_samples_percent_above_0_1': float(100*counts[c]/len(X)),
                         'training_percent_above_0_1': float(100*(Xtr[c] > .1).mean()) if c in Xtr else 0,
    ...
    config = {'seed': args.seed, 'n_iter': args.n_iter, 'group_map': group_map,
              'modeling_condition': 'Genomic/Microbiome features only', 'metadata_features': [], 'targets': TARGETS, 'blocks': BLOCKS,
              'filter': 'Use F(%) Family sheets. Strict sample-wise >0.1 percent; union across matched samples defines fixed schema. Mask <=0.1 to zero. No prevalence filter, root transform, renormalization, or additional feature selection.',
    ...
    save_json(out/'leakage_check.json', {'excluded_effluent_and_target_columns': excluded,
        'predictor_whitelist': X.columns.tolist(), 'excluded_metadata_columns': m.columns.tolist(),
        'reason': 'All metadata fields are excluded from predictors; only BAC_F:: and ARC_F:: features are allowed.', 'other_identity_columns_excluded': ['SampleID','Site','SiteGroup','No','Round'],
        'checks': ['No target or effluent predictor', 'Disjoint site groups in holdout and each CV fold',
                   'Fixed sample-wise abundance mask; imputation, scaling and resampling fitted within training folds',
    ...
    for name, space in SPACES.items():
        ...
        pd.DataFrame(final.preprocessor.transform(final.selector.transform(Xtr)), index=Xtr.index,
...
def enrich_report(out):
    assignment = pd.read_csv(out/'split_and_cv_assignment.csv')
    coverage = assignment.groupby(['SiteGroup','Site']).agg(
        samples=('Season','size'), seasons=('Season', lambda v: '|'.join(v)))
    coverage['missing_seasons'] = coverage.seasons.map(
        lambda v: '|'.join(s for s in ['Summer','Fall','Winter','Spring'] if s not in v.split('|')))
    coverage.to_csv(out/'samples_per_site.csv')
    dist = pd.read_csv(out/'label_distribution.csv')
    dist = dist[dist.scope.isin(['train','test'])].pivot(index='target',columns='scope',values='positive')
    text = ['\n## Train/test label distribution\n', '| Warning | Training positives (109) | Test positives (29) |', '|---|---:|---:|']
    text += [f'| {t} | {int(dist.loc[t,"train"])} | {int(dist.loc[t,"test"])} |' for t in TARGETS]
    text += ['\n## Seasonal coverage exceptions\n', '| Site | Site group | Included samples | Missing seasons |', '|---|---|---:|---|']
    text += [f'| {site} | {group} | {r.samples} | {r.missing_seasons.replace(chr(124), chr(44)+chr(32))} |' for (group,site),r in coverage.iterrows() if r.missing_seasons]
    selection = pd.read_csv(out/'cv_model_selection.csv')
    leaders = {metric: selection.loc[selection[metric].idxmax(), 'Model'] for metric in METRICS}
    text += ['\nCV selection metric leaders (MCC at 0.5): '+json.dumps(leaders)+'.']
...
def evaluate(out, m, X, y, test, fitted, selected, cv_table, audit, args):
    ...
    fig, ax = plt.subplots(figsize=(7, 6))
    for name in fitted:
        saved = pd.read_csv(out/f'{name}_test_predictions.csv', float_precision='round_trip')
    ...
    report = '\n'.join(['# Genomic/microbiome-only prediction of the five anaerobic digestion warning conditions',
        ...
        '\n## Reproducibility',
        f'Run `python ML/train_family_point1.py --seed {args.seed} --n-iter {args.n_iter}`. See run_config.json for input hashes, versions and search spaces.',
    ...
    leaders = {metric: test_table.loc[test_table[metric].idxmax(), 'Model'] for metric in METRICS}
    report += '\nTest metric leaders: ' + json.dumps(leaders) + '.\n'
    report += ('One model leads all three test metrics.\n' if len(set(leaders.values())) == 1 else 'The test metrics favor different models; there is no unanimous best test model. This comparison does not change CV selection.\n')
    ...
    if len(warning_frame):
        counts = warning_frame.groupby(['Model','category']).size()
        report += '\nFit warning counts: ' + str(counts.to_dict()) + '. See fit_warnings.csv; iteration-limit fits may not have converged.\n'
