# Multi-Label Classification Analysis for Anaerobic Digestion Warning Prediction — Order Level
...
**IMPORTANT:** This is a new analysis at the **Order taxonomic level**. Do NOT reuse, modify, or inherit the previous Family-level or Genus-level feature matrices, feature lists, preprocessing results, model results, hyperparameters, thresholds, or conclusions. Reconstruct the microbiome features and perform the analysis specifically for the Order-level dataset.
...
# 4. Taxonomic Feature Definition — ORDER LEVEL
...
This is an **Order-level analysis**.
...
* bacterial Order features
* archaeal Order features
...
1. Identify all bacterial **Order** taxa with relative abundance > 0.1%.
2. Identify all archaeal **Order** taxa with relative abundance > 0.1%.

Construct the final feature set as the **union of all bacterial and archaeal Order taxa that exceed 0.1% in at least one sample**.

After constructing this union feature list:

* if an Order has relative abundance > 0.1% in a given sample, use its actual relative abundance value;
* if an Order is present but has relative abundance ≤ 0.1%, represent it as `0`;
* if an Order is absent, represent it as `0`.

This produces a fixed microbiome feature matrix containing bacterial Order and archaeal Order columns across all samples.
...
Confirm that the final predictor matrix contains **only bacterial Order and archaeal Order microbiome features**.
...
3. Selected bacterial **Order** taxa satisfying sample-specific >0.1% rule
4. Selected archaeal **Order** taxa satisfying sample-specific >0.1% rule
...
* sample-specific `>0.1%` Order feature-construction rule
* preprocessing steps
...
> **How well can microbiome composition alone, represented at the Order level, predict the five anaerobic digestion warning indicators simultaneously?**
...
* which bacterial and archaeal Order features contribute most strongly to prediction.
...
* Bacteria at **Order level**
* Archaea at **Order level**
* **Relative abundance > 0.1%**
...
This analysis is specifically an **Order-level analysis**.

The following specification overrides any previous Family-level, Genus-level, Class-level, or Phylum-level configuration:

**Bacteria: Order levels (relative abundance > 0.1%)**

**Archaea: Order levels (relative abundance > 0.1%)**
...
The only allowed microbiome predictors are **bacterial Order and archaeal Order features** constructed using the sample-specific `>0.1%` relative-abundance rule.

Run this as a **new independent Order-level analysis** rather than modifying the previous Family-level or Genus-level analysis.
