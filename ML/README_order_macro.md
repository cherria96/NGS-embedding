# Order microbiome-only prediction with five-label macro metrics

Run code: [train_order_macro.py](train_order_macro.py). Results: [report.md](results_order_macro/report.md).
...
/home/best/anaconda3/envs/tsf-ad/bin/python ML/train_order_macro.py --seed 42 --n-iter 8

OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 MPLCONFIGDIR=/tmp/ngs-mpl \
/home/best/anaconda3/envs/tsf-ad/bin/python -m unittest discover -s ML -p test_order_macro.py

/home/best/anaconda3/envs/tsf-ad/bin/python ML/verify_order_macro.py
```
...
Order integration is implemented in `order_macro_data.py`; stateless splitting and search utilities are local to `order_macro_utils.py`;
no prior model scores or selected taxa are read. O(%) sheets supply original Order
percentages; taxa are checked against OTUs Order annotations. Workbook IDs are
...
All supplied Order annotation bins, including unidentified/off-domain annotations,
...
- `selected_taxa.csv`, `selected_Bacteria_orders.csv`, `selected_Archaea_orders.csv`,
  `final_feature_list.txt`, `taxa_audit.csv`, `cv_feature_columns.csv`: full feature
  lists, domains, Order level, per-taxon counts, and training/fold inclusion.
- `sample_audit.csv`, `sample_id_mapping.csv`, `sample_id_corrections.csv`,
  `file_structure.csv`, `order_source_counts.csv`, `samples_per_site.csv`,
...
- `*.joblib`, `*_preprocessing.joblib`: trained models and preprocessing. Load with
  ML on PYTHONPATH and supply raw Order percentages, not standardized values.
...
This independent run reads no prior result directories, model bundles, chosen hyperparameters, thresholds, feature lists or matrices. Search spaces are prespecified; all candidate fits and selections are rerun on Order data. The fixed seed may reproduce site assignments seen in earlier analyses; this does not constitute a new external cohort.
