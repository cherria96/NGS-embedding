...
def enrich(out):
    ...
    text+='Run test_class_macro.py for five protocol unit tests and verify_class_macro.py for independent verification of all 12 saved prediction-table metrics, source masks, final/fold unions and site separation, without new test predictions. See verification.json for the completed verification status.\n'
    ...
    for filename in ['feature_importance_per_warning.csv','feature_importance_macro.csv']:
        ...
        frame['taxonomic_level']='Class'
        frame.to_csv(out/filename,index=False)
    config=json.loads((out/'run_config.json').read_text())
    code=Path(__file__).resolve().parent
    names=['class_macro_models.py','class_macro_data.py','train_class_macro.py','report_class_macro.py','verify_class_macro.py','class_macro_utils.py']
    config['code_sha256']={n:hashlib.sha256((code/n).read_bytes()).hexdigest() for n in names}
    (out/'run_config.json').write_text(json.dumps(config,indent=2)+'\n')
    path.write_text(text)

if __name__=='__main__':enrich(Path(__file__).resolve().parent/'results_class_macro')
