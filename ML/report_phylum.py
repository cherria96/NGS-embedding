...
def build(out):
    ...
    config['feature_union_scope']='138 matched samples; fixed predefined threshold rule only; includes six columns with no training values above threshold.'
    ...
    counts['source_taxa']=counts.block.map({'BAC_P':60,'ARC_P':15})
    ...
    text=['# Phylum-only multi-label warning prediction',
...
if __name__=='__main__': build(Path(__file__).resolve().parent/'results_phylum')
