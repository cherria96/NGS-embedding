...
Do NOT use Phylum, Class, Order, Family, or Species level features in this analysis.
...
* bacterial Genus features
* archaeal Genus features
...
1. Identify all bacterial **Genus** taxa with relative abundance > 0.1%.
2. Identify all archaeal **Genus** taxa with relative abundance > 0.1%.

Construct the final feature set as the **union of all bacterial and archaeal Genus taxa that satisfy the >0.1% criterion in at least one sample**.

After the union feature list is constructed:

* if a Genus has relative abundance > 0.1% in a given sample, use its actual relative abundance value;
* if a Genus is present but has relative abundance ≤ 0.1%, represent it as `0`;
* if a Genus is absent, represent it as `0`.

The resulting feature matrix must therefore contain a fixed set of bacterial Genus and archaeal Genus columns across all samples.
...
Report which **bacterial Genus** and **archaeal Genus** features contribute most strongly to prediction where reliable interpretation is possible.
...
3. Selected bacterial **Genus** taxa satisfying the sample-specific >0.1% rule
4. Selected archaeal **Genus** taxa satisfying the sample-specific >0.1% rule
...
7. Taxonomic level for each feature:
   ...
   * Genus
8. Number of samples in which each Genus exceeded 0.1%
...
* sample-specific >0.1% Genus feature-construction rule
* preprocessing steps
...
> **How well can microbiome composition alone, represented at the Genus level, predict the five anaerobic digestion warning indicators simultaneously?**
...
* which bacterial and archaeal **Genus** features contribute most strongly to prediction.
...
## Final Taxonomic Specification — IMPORTANT

For this analysis, the taxonomic feature resolution is **Genus level for both domains**:

**Bacteria: Genus levels (relative abundance > 0.1%)**

**Archaea: Genus levels (relative abundance > 0.1%)**

This Genus-level specification overrides any previous Family-, Order-, Class-, or Phylum-level configuration.
...
Do not use Phylum-level features in this analysis.
Do not use Species-level features in this analysis.

The only microbiome taxonomic predictors allowed are bacterial Genus and archaeal Genus features constructed using the sample-specific `>0.1%` relative-abundance rule.
