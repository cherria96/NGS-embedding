...
class SampleAbundanceTaxa(BaseEstimator, TransformerMixin):
    """Fixed input schema; sample-wise strict >0.1 percent mask, no fitted selection."""
    ...
    def fit(self, X, y=None):
        if any(not c.startswith(('BAC_F::', 'ARC_F::')) for c in X.columns):
            raise ValueError('Microbiome-only input must contain Family features exclusively')
        self.selected_ = {b: sorted(c for c in X.columns if c.startswith(b+'::')) for b in BLOCKS}
