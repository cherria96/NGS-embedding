...
def enrich(out):
    ...
    text+='Run test_order_macro.py for five protocol unit tests and verify_order_macro.py for independent verification of all 12 saved prediction-table metrics, source masks, final/fold unions and site separation, without new test predictions. See verification.json for the completed verification status.\n'
    ...
    for filename in ['feature_importance_per_warning.csv','feature_importance_macro.csv']:
        ...
        frame['taxonomic_level']='Order'
        frame.to_csv(out/filename,index=False)
    config=json.loads((out/'run_config.json').read_text())
    code=Path(__file__).resolve().parent
    names=['order_macro_models.py','order_macro_data.py','train_order_macro.py','report_order_macro.py','verify_order_macro.py','order_macro_utils.py']
    config['code_sha256']={n:hashlib.sha256((code/n).read_bytes()).hexdigest() for n in names}
    (out/'run_config.json').write_text(json.dumps(config,indent=2)+'\n')
    path.write_text(text)

if __name__=='__main__':enrich(Path(__file__).resolve().parent/'results_order_macro')
