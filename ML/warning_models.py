...
CATS = ['Season', 'substrate_type']
NUMS = ['HRT_d', 'T_C', 'Q_total_Tpy'] + ['substrate_avg_' + x for x in ['pH', 'ALK', 'VS', 'COD', 'sCOD', 'TC', 'Protein', 'Lipid', 'TVFAs', 'HAc', 'HPro']]
BLOCKS = {'BAC_P': 10, 'BAC_G': 16, 'ARC_P': 10, 'ARC_G': 16}
...
class SampleTopTaxa(BaseEstimator, TransformerMixin):
    """Rank against the full supplied rank table, then project to training union.

    Ties (including zero abundance) break alphabetically. No renormalization.
    """
    def mask(self, X, block):
        ...
        order = np.argsort(-values, axis=1, kind='stable')[:, :BLOCKS[block]]
        mask = np.zeros(values.shape, dtype=bool)
        np.put_along_axis(mask, order, True, axis=1)
        return pd.DataFrame(mask, index=X.index, columns=columns)

    def fit(self, X, y=None):
        self.selected_ = {}
        self.counts_ = {}
        for block in BLOCKS:
            counts = self.mask(X, block).sum()
            self.selected_[block] = counts[counts > 0].index.tolist()
            self.counts_[block] = counts
        self.columns_ = CATS + NUMS + sum(self.selected_.values(), [])
        return self

    def transform(self, X):
        result = X[CATS + NUMS].copy()
        parts = [result]
        for block, columns in self.selected_.items():
            mask = self.mask(X, block)
            parts.append(X[columns].where(mask[columns], 0))
        return pd.concat(parts, axis=1)[self.columns_]
...
def preprocessing(X):
    numeric_cols = [c for c in X.columns if c not in CATS]
    return ColumnTransformer([
        ('numeric', Pipeline([('impute', SimpleImputer(strategy='median', keep_empty_features=True)), ('scale', StandardScaler())]), numeric_cols),
        ('categorical', Pipeline([('impute', SimpleImputer(strategy='most_frequent', keep_empty_features=True)), ('encode', OneHotEncoder(handle_unknown='ignore', sparse_output=False))]), CATS)
...
class WarningModel:
    """Binary relevance bundle, including taxa selection and preprocessing.

    SVM sigmoid calibration uses site-grouped out-of-fold margins, with feature
    selection/preprocessing refitted within each calibration training fold.
    """
    ...
    def fit(self, X, y, groups):
        self.selector = SampleTopTaxa().fit(X)
        selected = self.selector.transform(X)
        self.preprocessor = preprocessing(selected).fit(selected)
        ...
        if self.name == 'SVM':
            ...
            for train, valid in GroupKFold(n_splits=min(3, len(set(groups)))).split(X, y, groups):
                selector = SampleTopTaxa().fit(X.iloc[train])
