#!/usr/bin/env python3
"""Site-grouped joint warning comparison; see README_genomic.md."""
...
from genomic_models import TARGETS, CATS, NUMS, BLOCKS, SampleAbundanceTaxa, WarningModel, numeric
...
def inspect(data_dir, out, group_map):
    m = pd.read_csv(data_dir/'metadata.csv', usecols=['SampleID','Site']+TARGETS,
    ...
    return m,X,m[TARGETS].to_numpy(int),label_rows,[]
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=ROOT/'ML/results_genomic')
    ...
    source_assignment = ROOT/'ML/results_multilabel/split_and_cv_assignment.csv'
    assignment = pd.read_csv(source_assignment, dtype={'SampleID':str}).set_index('SampleID')
    if set(assignment.index) != set(m.index):
        raise ValueError('Samples differ from locked original assignment')
    assignment = assignment.loc[m.index, ['Site','SiteGroup','split','cv_validation_fold']]
    assert (assignment.Site == m.Site).all() and (assignment.SiteGroup == m.SiteGroup).all()
    train=np.flatnonzero(assignment.split.to_numpy()=='train')
    test=np.flatnonzero(assignment.split.to_numpy()=='test')
    Xtr,ytr,gtr=X.iloc[train],y[train],groups[train]
    fold_ids=assignment.iloc[train].cv_validation_fold.to_numpy(int)
    cv=[(np.flatnonzero(fold_ids!=fold),np.flatnonzero(fold_ids==fold)) for fold in range(1,6)]
    ...
    save_json(out/'leakage_check.json', {'excluded_effluent_and_target_columns': excluded,
        'predictor_whitelist': list(X.columns), 'other_identity_columns_excluded': ['SampleID','Site','SiteGroup','No','Round'],
...
def evaluate(out, m, X, y, test, fitted, selected, cv_table, audit, args):
    ...
    report = '\n'.join(['# Genomic-only joint five-label warning prediction',f'CV-selected model: **{selected}**.',
        ...
        '\n## Data and methods','\n\n'.join(audit), '\n## Reproducibility',
        f'Run `python ML/train_genomic.py --seed {args.seed} --n-iter {args.n_iter}` with the versions in run_config.json.',
    ...
    report += '\nGenomic taxa contribution analysis: see genomic_importance/report.md and genomic_importance/ranked_taxa.csv after running importance_genomic.py.\n'
