"""Audited Genus workbook integration; metadata is never a predictor."""
import numpy as np
import pandas as pd
from genus_macro_models import TARGETS, CATS, NUMS, numeric
from train_family_point1 import label_report
...
def inspect(data_dir, out, group_map):
    ...
    for domain in ['BAC', 'ARC']:
        ...
        frame = pd.read_excel(book, sheet_name='G(%)').set_index('Genus')
        if frame.index.isna().any() or frame.index.duplicated().any():
            raise ValueError('Missing/duplicate Genus names')
        if set(frame.index) != set(otu.Genus.fillna('unidentified')):
            raise ValueError('Genus sheet and OTU Genus annotation disagree')
        frame = frame.apply(pd.to_numeric, errors='raise')
        if not np.isfinite(frame.values).all() or (frame.values < 0).any():
            raise ValueError('Missing or invalid Genus percentage: cannot assume absence')
        if not np.allclose(frame.sum(axis=0), 100, atol=.1):
            raise ValueError('Genus percentages do not sum to approximately 100')
        ...
        for full in original:
            ...
            mappings.append({'domain': domain, 'rank': 'Genus', 'workbook_sample': full,
        ...
        frame.columns = ids
        frame = frame.T
        genus_source.append({'domain': domain, 'original_OTUs': len(otu),
                              'unique_Genera': len(frame.columns)})
        frame.columns = [f'{domain}_G::{taxon}' for taxon in frame.columns]
        sheets[f'{domain}_G'] = frame
        book.close()
    pd.DataFrame(genus_source).to_csv(out/'genus_source_counts.csv', index=False)
    ...
    assert all(c.startswith(('BAC_G::', 'ARC_G::')) for c in X.columns)
