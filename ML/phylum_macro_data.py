"""Audited Phylum workbook integration; metadata is never a predictor."""
import numpy as np
import pandas as pd
from phylum_macro_models import TARGETS, CATS, NUMS, numeric
from phylum_macro_utils import label_report
...
def inspect(data_dir, out, group_map):
    ...
    for domain in ['BAC', 'ARC']:
        ...
        frame = pd.read_excel(book, sheet_name='P(%)').set_index('Phylum')
        if frame.index.isna().any() or frame.index.duplicated().any():
            raise ValueError('Missing/duplicate Phylum names')
        if set(frame.index) != set(otu.Phylum.fillna('unidentified')):
            raise ValueError('Phylum sheet and OTU Phylum annotation disagree')
        frame = frame.apply(pd.to_numeric, errors='raise')
        if not np.isfinite(frame.values).all() or (frame.values < 0).any():
            raise ValueError('Missing or invalid Phylum percentage: cannot assume absence')
        if not np.allclose(frame.sum(axis=0), 100, atol=.1):
            raise ValueError('Phylum percentages do not sum to approximately 100')
        ...
        for full in original:
            ...
            mappings.append({'domain': domain, 'rank': 'Phylum', 'workbook_sample': full,
        ...
        frame.columns = ids
        frame = frame.T
        phylum_source.append({'domain': domain, 'original_OTUs': len(otu),
                              'unique_Phyla': len(frame.columns)})
        frame.columns = [f'{domain}_P::{taxon}' for taxon in frame.columns]
        sheets[f'{domain}_P'] = frame
        book.close()
    pd.DataFrame(phylum_source).to_csv(out/'phylum_source_counts.csv', index=False)
    ...
    assert all(c.startswith(('BAC_P::', 'ARC_P::')) for c in X.columns)
