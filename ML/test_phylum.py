...
import numpy as np
import pandas as pd
from phylum_models import SampleAbundanceTaxa, WarningModel
from train_phylum import metrics, partition

class PhylumTests(unittest.TestCase):
    def test_threshold_and_fixed_columns(self):
        x=pd.DataFrame({'BAC_P::a':[.1,.101,0], 'ARC_P::b':[0,0,.3], 'Season':['Summer']*3})
        s=SampleAbundanceTaxa().fit(x.iloc[:2])
        self.assertEqual(s.columns_, ['BAC_P::a','ARC_P::b'])
    ...
    def test_fold_local_scaling_and_constant_label(self):
        x=pd.DataFrame({'BAC_P::a':[.2,.3,.4,.5,.6,.7], 'ARC_P::b':[0]*6})
