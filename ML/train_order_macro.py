#!/usr/bin/env python3
"""Genomic Order binary relevance with equal-weight five-label macro metrics."""
...
from order_macro_models import TARGETS, BLOCKS, SampleAbundanceTaxa, WarningModel
from order_macro_data import inspect
from order_macro_utils import partition, label_report, predictions, save_json, SPACES, ROOT
...
def main():
    ...
    parser.add_argument('--output-dir',type=Path,default=ROOT/'ML/results_order_macro')
    ...
    taxa=pd.DataFrame([{'feature':c,'taxon':c.split('::')[1],'domain':'Bacteria' if c.startswith('BAC') else 'Archaea','taxonomic_level':'Order',
    ...
    for domain in ['Bacteria','Archaea']:
        taxa[(taxa.domain==domain)&taxa.included_in_final_matrix].to_csv(out/f'selected_{domain}_orders.csv',index=False)
    ...
    save_json(out/'leakage_check.json',{'all_metadata_excluded':list(m.columns)+['SampleID'],'predictors':selector.columns_,
        'passed':['Only Order percentage columns in predictors','No identities, season, effluent, operation, substrate or warning predictors','Site groups disjoint','Training-only feature union and preprocessing'],
    ...
    report=['# Order microbiome-only multi-label warning prediction',f'CV-selected model: **{DISPLAY.get(selected,selected)}**. Selection was frozen before test evaluation.',
    ...
    report += ['## Limits','Sampling times do not establish prospective prediction. This is contemporaneous classification. Existing zeros with missing target-source chemistry mean not flagged, not confirmed normal. The deterministic seed-42 holdout appeared in earlier project analyses: isolated in this run, but not a previously unseen external cohort. Few sites and very few rare-warning positives make all comparisons uncertain. A macro mean weights all five warnings equally but does not imply consistent performance or exact five-label-vector recovery.',f'Recorded fit warnings: {len(caught_rows)}; consult fit_warnings.csv for convergence limitations.',
        '## Reproducibility',f'Run `python ML/train_order_macro.py --seed {args.seed} --n-iter {args.n_iter} --output-dir NEW_DIRECTORY`. See run_config.json for versions, source hashes and full search spaces. Load joblib with ML on PYTHONPATH and supply raw Order percentages; preprocessing and thresholds are inside the bundle.']
    (out/'report.md').write_text('\n\n'.join(report)+'\n')
    from report_order_macro import enrich
    enrich(out)
