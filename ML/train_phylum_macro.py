#!/usr/bin/env python3
"""Genomic Phylum binary relevance with equal-weight five-label macro metrics."""
...
from phylum_macro_models import TARGETS, BLOCKS, SampleAbundanceTaxa, WarningModel
from phylum_macro_data import inspect
from phylum_macro_utils import partition, label_report, predictions, save_json, SPACES, ROOT
...
def main():
    ...
    parser.add_argument('--output-dir',type=Path,default=ROOT/'ML/results_phylum_macro')
    ...
    prior=ROOT/'ML/results_family_macro/split_and_cv_assignment.csv'
    if prior.exists():
        reference=pd.read_csv(prior,index_col='SampleID')
        pd.testing.assert_frame_equal(assignment.sort_index().astype(str),reference.sort_index().astype(str).replace({'nan':'<NA>','1.0':'1','2.0':'2','3.0':'3','4.0':'4','5.0':'5'}),check_dtype=False)
    ...
    taxa=pd.DataFrame([{'feature':c,'taxon':c.split('::')[1],'domain':'Bacteria' if c.startswith('BAC') else 'Archaea','taxonomic_level':'Phylum',
    ...
    for domain in ['Bacteria','Archaea']:
        taxa[(taxa.domain==domain)&taxa.included_in_final_matrix].to_csv(out/f'selected_{domain}_phyla.csv',index=False)
    ...
    save_json(out/'leakage_check.json',{'all_metadata_excluded':list(m.columns)+['SampleID'],'predictors':selector.columns_,
        'passed':['Only Phylum percentage columns in predictors','No identities, season, effluent, operation, substrate or warning predictors','Site groups disjoint','Training-only feature union and preprocessing'],
    ...
    report=['# Phylum microbiome-only multi-label warning prediction',f'CV-selected model: **{DISPLAY.get(selected,selected)}**. Selection was frozen before test evaluation.',
    ...
    report += ['## Limits','Sampling times do not establish prospective prediction. This is contemporaneous classification. Existing zeros with missing target-source chemistry mean not flagged, not confirmed normal. The deterministic seed-42 holdout appeared in earlier project analyses: isolated in this run, but not a previously unseen external cohort. Few sites and very few rare-warning positives make all comparisons uncertain. A macro mean weights all five warnings equally but does not imply consistent performance or exact five-label-vector recovery.',f'Recorded fit warnings: {len(caught_rows)}; consult fit_warnings.csv for convergence limitations.',
        '## Reproducibility',f'Run `python ML/train_phylum_macro.py --seed {args.seed} --n-iter {args.n_iter} --output-dir NEW_DIRECTORY`. See run_config.json for versions, source hashes and full search spaces. Load joblib with ML on PYTHONPATH and supply raw Phylum percentages; preprocessing and thresholds are inside the bundle.']
    (out/'report.md').write_text('\n\n'.join(report)+'\n')
    from report_phylum_macro import enrich
    enrich(out)
