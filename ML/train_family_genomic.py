#!/usr/bin/env python3
"""Site-grouped joint warning comparison; see README_family_genomic.md."""
...
from family_genomic_models import TARGETS, CATS, NUMS, BLOCKS, SampleAbundanceTaxa, WarningModel, numeric
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=ROOT/'ML/results_family_genomic')
    ...
    previous = ROOT/'ML/results_family/split_and_cv_assignment.csv'
    if args.seed == 42 and previous.exists():
        prior = pd.read_csv(previous, index_col='SampleID')
        pd.testing.assert_frame_equal(assignment.sort_index(), prior.sort_index(), check_dtype=False)
    ...
    audit = [f'Matched samples: {len(m)}; site labels: {m.Site.nunique()}; groups: {len(set(groups))}.',
             ...
             'Numeric median imputation and z-score scaling fitted on training Family features only; no categorical predictors or metadata encoding.',
...
def evaluate(out, m, X, y, test, fitted, selected, cv_table, audit, args):
    ...
    report = '\n'.join(['# Genomic/microbiome-only prediction of the five anaerobic digestion warning conditions',
        ...
        '\n## Reproducibility',
        f'Run `python ML/train_family_genomic.py --seed {args.seed} --n-iter {args.n_iter}`. See run_config.json for input hashes, versions and search spaces.',
        'See split_and_cv_assignment.csv for every sample/site/fold, label_distribution.csv for label counts, selected_taxa.csv for the complete Family list, and README_family_genomic.md for pipeline details.'])
