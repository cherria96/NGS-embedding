#!/usr/bin/env python3
"""Site-grouped joint warning comparison; see README_phylum.md."""
...
from phylum_models import TARGETS, CATS, NUMS, BLOCKS, SampleAbundanceTaxa, WarningModel, numeric
...
def main():
    ...
    parser.add_argument('--output-dir', type=Path, default=ROOT/'ML/results_phylum')
    ...
    audit = [f'Matched samples: {len(m)}; site labels: {m.Site.nunique()}; groups: {len(set(groups))}.',
             ...
             'Only SampleID, Site, Season and five targets read from metadata; genomic IDs normalized and checked.',
             ...
             'No metadata predictors: only BAC phylum and ARC phylum percentages.',
    ...
    from report_phylum import build
    build(out)
...
def evaluate(out, m, X, y, test, fitted, selected, cv_table, audit, args):
    ...
    report = '\n'.join(['# Phylum-only joint five-label warning prediction',f'CV-selected model: **{selected}**.',
        ...
        '\n## Data and methods','\n\n'.join(audit), '\n## Reproducibility',
        f'Run `python ML/train_phylum.py --seed {args.seed} --n-iter {args.n_iter}` with the versions in run_config.json.',
