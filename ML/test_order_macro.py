...
from order_macro_models import SampleAbundanceTaxa, WarningModel
from train_order_macro import metrics, choose_threshold, select_joint

class MacroProtocolTests(unittest.TestCase):
    def test_training_union_strict_boundary(self):
        x=pd.DataFrame({'BAC_O::a':[.1,.10001,1.], 'ARC_O::b':[0,0,10.]})
        s=SampleAbundanceTaxa().fit(x.iloc[:2])
        self.assertEqual(s.columns_,['BAC_O::a'])
    ...
    def test_constant_training_labels(self):
        x=pd.DataFrame({'BAC_O::a':[0.,.2,.3,1.6]*5,'ARC_O::b':[.4,0.,.2,0.]*5})
