# Family microbiome-only prediction with five-label macro metrics

Run code: [train_family_macro.py](train_family_macro.py). Results: [report.md](results_family_macro/report.md).
...
/home/best/anaconda3/envs/tsf-ad/bin/python ML/train_family_macro.py --seed 42 --n-iter 8

OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 MPLCONFIGDIR=/tmp/ngs-mpl \
/home/best/anaconda3/envs/tsf-ad/bin/python -m unittest discover -s ML -p test_family_macro.py

/home/best/anaconda3/envs/tsf-ad/bin/python ML/verify_family_macro.py
```
...
The shared `inspect` and `partition` utilities in `train_family_point1.py` are reused;
no prior model scores or selected taxa are read. F(%) sheets supply original Family
percentages; taxa are checked against OTUs Family annotations. Workbook IDs are
...
All supplied Family annotation bins, including unidentified/off-domain annotations,
...
training portion, including CV and SVM calibration. The final training union has
329 columns; the 370-column all-sample union is a separate descriptive export and
...
- `selected_taxa.csv`, `selected_Bacteria_families.csv`, `selected_Archaea_families.csv`,
  `final_feature_list.txt`, `taxa_audit.csv`, `cv_feature_columns.csv`: full feature
  lists, domains, Family level, per-taxon counts, and training/fold inclusion.
- `sample_audit.csv`, `sample_id_mapping.csv`, `sample_id_corrections.csv`,
  `file_structure.csv`, `family_source_counts.csv`, `samples_per_site.csv`,
...
- `*.joblib`, `*_preprocessing.joblib`: trained models and preprocessing. Load with
  ML on PYTHONPATH and supply raw Family percentages, not standardized values.
