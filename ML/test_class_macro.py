...
from class_macro_models import SampleAbundanceTaxa, WarningModel
from train_class_macro import metrics, choose_threshold, select_joint

class MacroProtocolTests(unittest.TestCase):
    def test_training_union_strict_boundary(self):
        x=pd.DataFrame({'BAC_C::a':[.1,.10001,1.], 'ARC_C::b':[0,0,10.]})
        s=SampleAbundanceTaxa().fit(x.iloc[:2])
        self.assertEqual(s.columns_,['BAC_C::a'])
    ...
    def test_constant_training_labels(self):
        x=pd.DataFrame({'BAC_C::a':[0.,.2,.3,1.6]*5,'ARC_C::b':[.4,0.,.2,0.]*5})
