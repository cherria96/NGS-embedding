# Family-level multi-label warning prediction
...
Results are in [results_family/report.md](results_family/report.md).
...
/home/best/anaconda3/envs/tsf-ad/bin/python ML/train_family.py --seed 42 --n-iter 8

OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 \
MPLCONFIGDIR=/tmp/ngs-mpl XDG_CACHE_HOME=/tmp/ngs-cache \
/home/best/anaconda3/envs/tsf-ad/bin/python -m unittest discover -s ML -p test_family.py
```
...
## Metadata, missingness and leakage

Only the requested 16 metadata variables are predictors: Season, substrate_type,
HRT_d, T_C, Q_total_Tpy, and substrate_avg_{pH,ALK,VS,COD,sCOD,TC,Protein,Lipid,
TVFAs,HAc,HPro}. Input numeric ranges use midpoints; ± expressions use their
centers. Numeric coercions and missingness are saved. Numerical median imputation
(all-empty columns use zero), categorical most-frequent imputation, unknown-safe
one-hot encoding and z-score scaling are fitted within each training portion.
Scaling is applied consistently to all six models, including tree models.

All eff_* columns are excluded because they include direct target definitions
and other effluent measurements outside the predictor whitelist. All warning_*
columns are excluded from X because they are outcomes. SampleID, Site, SiteGroup,
No and Round are identifiers/grouping fields rather than predictors. Full excluded
lists are in leakage_check.json. The cleaned merge preserves source effluent
columns for audit; final_feature_matrix.csv contains only allowed predictors.
Supplied substrate averages are treated as input measurements. No target-derived
features are calculated. Existing warning labels are unchanged; missing source
measurements can yield zeros that mean unflagged rather than confirmed normal.
...
- file_structure.csv, sample_id_mapping.csv, sample_id_corrections.csv,
  sample_audit.csv, samples_per_site.csv, numeric_coercions.csv,
...
To load joblib bundles, make ML importable (`PYTHONPATH=ML`). Supply the raw
metadata and untransformed Family percentages named `BAC_F::Family` and
...
This run recorded 15 convergence warnings: two NNET CV fits and 13 GLMNET fits, including two final-label estimators. These limitations are retained in the report; the CV-selected Random Forest had no fit warnings.
