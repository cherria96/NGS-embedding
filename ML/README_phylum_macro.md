# Phylum microbiome-only prediction with five-label macro metrics

Run code: [train_phylum_macro.py](train_phylum_macro.py). Results: [report.md](results_phylum_macro/report.md).
...
/home/best/anaconda3/envs/tsf-ad/bin/python ML/train_phylum_macro.py --seed 42 --n-iter 8

OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 MPLCONFIGDIR=/tmp/ngs-mpl \
/home/best/anaconda3/envs/tsf-ad/bin/python -m unittest discover -s ML -p test_phylum_macro.py

/home/best/anaconda3/envs/tsf-ad/bin/python ML/verify_phylum_macro.py
```
...
Phylum integration is implemented in `phylum_macro_data.py`; stateless splitting and search utilities are local to `phylum_macro_utils.py`;
no prior model scores or selected taxa are read. P(%) sheets supply original Phylum
percentages; taxa are checked against OTUs Phylum annotations. Workbook IDs are
...
All supplied Phylum annotation bins, including unidentified/off-domain annotations,
...
- `selected_taxa.csv`, `selected_Bacteria_phyla.csv`, `selected_Archaea_phyla.csv`,
  `final_feature_list.txt`, `taxa_audit.csv`, `cv_feature_columns.csv`: full feature
  lists, domains, Phylum level, per-taxon counts, and training/fold inclusion.
- `sample_audit.csv`, `sample_id_mapping.csv`, `sample_id_corrections.csv`,
  `file_structure.csv`, `phylum_source_counts.csv`, `samples_per_site.csv`,
...
- `*.joblib`, `*_preprocessing.joblib`: trained models and preprocessing. Load with
  ML on PYTHONPATH and supply raw Phylum percentages, not standardized values.
...
This independent run reads no prior result directories, model bundles, chosen hyperparameters, thresholds, feature lists or matrices. Search spaces are prespecified; all candidate fits and selections are rerun on Phylum data. The fixed seed may reproduce site assignments seen in earlier analyses; this does not constitute a new external cohort.

## Cross-resolution comparison and all-model interpretation

The Phylum training script checks its deterministic site/fold assignments against
`results_family_macro/split_and_cv_assignment.csv` when available, before fitting.
It reads only assignments at that stage, not earlier performance or fitted objects.
After completing the run, execute:

```bash
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 MPLCONFIGDIR=/tmp/ngs-mpl \
/home/best/anaconda3/envs/tsf-ad/bin/python ML/supplement_phylum_macro.py
```

This independently loads the Phylum fitted models and computes three-repeat
training-resubstitution permutation importance for every feature and all three
macro metrics, for all six models. It does not predict or shuffle test rows, fit
models, change thresholds, or select features. These diagnostic importance scores
can be optimistic and do not establish significance, biological mechanism or causality.

`test_warning_summary.csv` joins each flag's test metrics, optimized threshold,
positive count and negative count. `resolution_comparison_all_models.csv` contains
all 30 frozen model/resolution results; `resolution_comparison_cv_selected.csv`
compares each resolution's independently CV-selected model. Assignment and input
hash equality are required across all five resolutions. Earlier test scores are
read only for this post-evaluation descriptive comparison, never for Phylum tuning.
The report includes the comparison and leading permutation contributions.
