...
from phylum_macro_models import SampleAbundanceTaxa, WarningModel
from train_phylum_macro import metrics, choose_threshold, select_joint

class MacroProtocolTests(unittest.TestCase):
    def test_training_union_strict_boundary(self):
        x=pd.DataFrame({'BAC_P::a':[.1,.10001,1.], 'ARC_P::b':[0,0,10.]})
    ...
    def test_constant_training_labels(self):
        x=pd.DataFrame({'BAC_P::a':[0.,.2,.3,1.6]*5,'ARC_P::b':[.4,0.,.2,0.]*5})
