...
class SampleAbundanceTaxa(BaseEstimator, TransformerMixin):
    """Training-only union of sample-wise strict >0.1 percent Order abundances."""
    ...
    def fit(self, X, y=None):
        if any(not c.startswith(('BAC_O::', 'ARC_O::')) for c in X.columns):
            raise ValueError('Microbiome-only input must contain Order features exclusively')
