...
def enrich(out):
    ...
    consistency=test.groupby('target').MCC.mean().sort_values(ascending=False)
    text+=f'The largest mean test MCC across the six models is for {consistency.index[0]} ({consistency.iloc[0]:.3f}), and the smallest is for {consistency.index[-1]} ({consistency.iloc[-1]:.3f}). This is descriptive only, not another model-selection criterion. '
    text+='Inspect both per-warning ranking areas and thresholded MCC: ranking one rare positive highly need not give a reliable threshold. Acid accumulation has very few examples, so even perfect test ranking would not establish robust generalization. The macro results do not establish dependable performance on all five warnings.\n\n'
    ...
    text+='Run test_phylum_macro.py for five protocol unit tests and verify_phylum_macro.py for independent verification of all 12 saved prediction-table metrics, source masks, final/fold unions and site separation, without new test predictions. See verification.json for the completed verification status.\n'
    # Persist explicit annotation level and code provenance without model evaluation.
    for filename in ['feature_importance_per_warning.csv','feature_importance_macro.csv']:
        frame=pd.read_csv(out/filename)
        frame['taxonomic_level']='Phylum'
        frame.to_csv(out/filename,index=False)
    config=json.loads((out/'run_config.json').read_text())
    code=Path(__file__).resolve().parent
    names=['phylum_macro_models.py','phylum_macro_data.py','train_phylum_macro.py','report_phylum_macro.py','verify_phylum_macro.py','phylum_macro_utils.py']
    config['code_sha256']={n:hashlib.sha256((code/n).read_bytes()).hexdigest() for n in names}
    (out/'run_config.json').write_text(json.dumps(config,indent=2)+'\n')
    path.write_text(text)

if __name__=='__main__':enrich(Path(__file__).resolve().parent/'results_phylum_macro')
