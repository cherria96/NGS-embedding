# Class-only multi-label warning analysis

Results and complete methods: [results_class/report.md](results_class/report.md).

From the repository root:

```bash
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 MPLCONFIGDIR=/tmp/ngs-mpl \
/home/best/anaconda3/envs/tsf-ad/bin/python ML/train_class.py --seed 42 --n-iter 8

OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 MPLCONFIGDIR=/tmp/ngs-mpl \
/home/best/anaconda3/envs/tsf-ad/bin/python -m unittest discover -s ML -p test_class.py
```

Inputs are data/final/metadata.csv, Dat_BAC.xlsx and Dat_ARC.xlsx. Only the
C(%) sheets provide predictors. A strict per-sample >0.1 percent mask is
...
Serialized models require class_models on the import path: use PYTHONPATH=ML
...
To rebuild only the narrative from existing outputs, run ML/report_class.py.
