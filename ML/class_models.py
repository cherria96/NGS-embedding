...
CATS = []
NUMS = []
BLOCKS = {'BAC_C': 'Class', 'ARC_C': 'Class'}
...
class SampleAbundanceTaxa(BaseEstimator, TransformerMixin):
    """Strict per-sample mask on the predefined union columns; no fitted feature selection."""
    ...
    def fit(self, X, y=None):
        ...
        for block in BLOCKS:
            ...
            self.selected_[block] = counts.index.tolist()
...
def preprocessing(X, scale=True):
    ...
    return ColumnTransformer([
        ('numeric', Pipeline([('impute', SimpleImputer(strategy='median', keep_empty_features=True)), ('scale', StandardScaler() if scale else 'passthrough')]), numeric_cols),
...
class WarningModel:
    ...
    def fit(self, X, y, groups):
        ...
        self.preprocessor = preprocessing(selected, scale=self.name not in ('RandomForest', 'XGBoost')).fit(selected)
