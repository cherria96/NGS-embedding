...
CATS = []
NUMS = []
BLOCKS = {'BAC_G': 'Genus', 'ARC_G': 'Genus'}
...
class SampleAbundanceTaxa(BaseEstimator, TransformerMixin):
    """Training-only union of sample-wise strict >0.1 percent Genus abundances."""
    ...
    def fit(self, X, y=None):
        if any(not c.startswith(('BAC_G::', 'ARC_G::')) for c in X.columns):
            raise ValueError('Microbiome-only input must contain Genus features exclusively')
