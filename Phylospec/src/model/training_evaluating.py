...
# Calculate ROC AUC score for binary, multi-class, or multi-label classification
def calculate_roc_auc(y_true, y_scores, num_classes, multi_label=False):
    if multi_label:
        roc_auc = []
...
def cv_train_and_evaluate(model, train_loader, test_loader, criterion, optimizer, conv_order, data, leaf_to_species,
                          node_weights, num_epochs=10, num_classes=2, multi_label=False):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loader:
            if isinstance(criterion, torch.nn.BCEWithLogitsLoss):
                labels = labels.float()
                if labels.dim() == 1:
                    labels = labels.unsqueeze(1)  # Binary: [batch_size] -> [batch_size, 1]
