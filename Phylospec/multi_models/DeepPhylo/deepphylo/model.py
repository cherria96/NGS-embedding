...
class DeepPhylo_multi_label_logits(nn.Module):
    """Same architecture as DeepPhylo_multi_label but without the final
    Sigmoid, so it emits raw logits for use with nn.BCEWithLogitsLoss(pos_weight=...)
    -- the Sigmoid+MSELoss combination in DeepPhylo_multi_label has no way to
    up-weight the rare "Warning" class, which matters a lot given some flags
    have <5% positives."""
    def __init__(self, hidden_size, embeddings, kernel_size_conv=7, kernel_size_pool=4, dropout_conv=0.2, activation=nn.ReLU(), n_labels=4):
        ...
        self.fc_phy = nn.Linear(self.embedding_dim, hidden_size)
        self.fc_pred = nn.Linear(hidden_size, n_labels)
