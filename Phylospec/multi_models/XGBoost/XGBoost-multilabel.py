"""
XGBoost baseline for multi-label warning prediction (5 flags at once). Same
CLI/CSV contract as RF-multilabel.py, with one difference: XGBClassifier's
imbalance knob (scale_pos_weight) is a single scalar, so -- unlike RF/SVM/KNN,
which fit one class_weight='balanced' model across all 5 outputs at once via
MultiOutputClassifier -- this script fits 5 SEPARATE XGBClassifier instances,
each with its own scale_pos_weight = min(n_neg/n_pos, --pos-weight-cap)
computed from that flag's TRAINING fold only (mirrors the pos_weight tensor
DeepPhylo/PhyloSpec use, task Sec 5A.4, and actually uses --pos-weight-cap
instead of accepting-and-ignoring it).
"""
import argparse
import os
import sys
...
def scale_pos_weight(y_col, cap):
    n_pos = int(y_col.sum())
...
def fit_predict_all_flags(X_train, y_train, X_eval, pos_weight_cap, seed):
    ...
    for i in range(len(FLAGS)):
        spw = scale_pos_weight(y_train[:, i], pos_weight_cap)
        clf = XGBClassifier(
            n_estimators=200,
            max_depth=3,
            learning_rate=0.1,
...
def main(train_csv, test_csv, out_path, domain, seed=42, val_csv=None,
    ...
    if scores_out_dir:
        ...
        if val_csv:
            ...
            X_val = val_df.drop(columns=["SampleID"] + FLAGS).values
    ...
    save_results(result, "XGBoost", domain, out_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run multi-label XGBoost.")
    ...
    parser.add_argument("--pos-weight-cap", type=float, default=10.0,
                         help="cap on per-flag scale_pos_weight = n_neg/n_pos, "
