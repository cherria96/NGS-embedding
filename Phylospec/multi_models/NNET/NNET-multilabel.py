"""
Small feedforward neural net (scikit-learn MLPClassifier) baseline for
multi-label warning prediction (5 flags at once). Same CLI/CSV contract as
RF-multilabel.py. MLPClassifier natively handles a multilabel-indicator y and
its predict_proba already returns a single (n_samples, n_labels) array (not a
list), which is the second branch predict_proba_matrix handles.
"""
import argparse
import os
import sys
...
def main(train_csv, test_csv, out_path, domain, seed=42, val_csv=None, scores_out_dir=None):
    ...
    save_results(result, "NNET", domain, out_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run multi-label NNET (sklearn MLP).")
    ...
    parser.add_argument("--pos-weight-cap", type=float, default=None,
                         help="accepted for CLI uniformity with the other model "
                              "scripts; unused -- MLPClassifier has no "
                              "class-weighting knob (sklearn does not support "
                              "class_weight for MLPClassifier)")
